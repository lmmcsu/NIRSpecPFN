...
output_dir = os.path.join(os.path.dirname(__file__), f"tacator_{target_property}_significance_results")
...
# ==============================================================
# 3. Fixed train/test split based on subset column (C/M vs T)
# ==============================================================
print("\n>>> Creating fixed train/test split using subset column")

# Use the first sheet as reference (all sheets have the same sample order)
ref_sheet = all_preprocessing_sheets[0]  # "Raw Spectra"
df_ref = pd.read_excel(file_path, sheet_name=ref_sheet)

# Extract subset column (second column, index 1)
subset = df_ref.iloc[:, 1]

# Determine training and test masks
...
# ==============================================================
dataset_cache = {}
y_all_ref = df_ref[target_property].to_numpy()
y_train_fixed = y_all_ref[train_idx]
y_test_fixed = y_all_ref[test_idx]

for sheet in all_preprocessing_sheets:
    df = pd.read_excel(file_path, sheet_name=sheet)
    X_all = df.iloc[:, 2:102].to_numpy()   # spectral columns
...
if selection_mode == "global":
    selected_config = {
        model_name: best_global_row
        for model_name in model_names
    }
    print("\n>>> Selection mode: global")
    print(
        f"Global best: {best_global_row['Preprocessing']} | {best_global_row['Model']} | "
        f"RMSECV={best_global_row['RMSECV']:.4f}"
    )
else:
...
def evaluate_pfn(X_tr, y_tr, X_te, y_te, seed):
...
for i in range(n_bootstrap):
    ...
    rmse = evaluate_pfn(X_boot, y_boot, X_test, y_test_fixed, seed=i)
...
excel_path = os.path.join(output_dir, f"tacator_{target_property}_bootstrap_rmse_results.xlsx")
...
print("\nBootstrap RMSE results saved to:", excel_path)
