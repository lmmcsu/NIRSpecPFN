...
# Load the data
data_path = 'D:/A/CSU/数据集/corn_xlsl/mp5_corn.xlsx'
df = pd.read_excel(data_path)
spectra = df.iloc[:, :700].values  
y = df.iloc[:, 701].values.ravel()
...
for fold, (train_idx, test_idx) in enumerate(kf.split(spectra), 1):
    ...
    # Dataprocessing
    preprocess_method = 'Savitzky-Golay 1st Derivative'
    X_train_sg = savgol_filter(X_train, window_length=11, polyorder=2, deriv=1, axis=1)
    ...
    # TabPFN-default
    tabpfn_regressor = TabPFNRegressor(device=device, random_state=42)
    ...
    # PLSR-tuning
    param_grid = {'n_components': list(range(5, 21))} # n_components from 5 to 20
    pls = PLSRegression()
...
y_col_index = spectra.shape[1]    # column index of label                    
