...
for sheet_name in sheets_to_run:
    ...
    # Kennard-Stone partition for all methods under current preprocessing
    X_train_ks, X_test_ks, y_train_ks, y_test_ks = ks_split(X, y, test_size=0.3)

    pfn_results = get_pfn_metrics(X_train_ks, y_train_ks, X_test_ks, y_test_ks, seed=42)
    plsr_results = get_plsr_metrics(X_train_ks, y_train_ks, X_test_ks, y_test_ks, seed=42)
    svr_results = get_svr_metrics(X_train_ks, y_train_ks, X_test_ks, y_test_ks, seed=42)
    ...
    plot_true_vs_pred(y_test_ks, results_by_method, sheet_name, plot_path)
