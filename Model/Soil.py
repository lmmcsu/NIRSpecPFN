...
# Available Sheets: "Raw spectral", "Smoothing", "MSC", "SNV", "1-derivative", "2-derivative", "Mean center", "S-G+SNV", "all"
selected_sheet = "all"
target_property = "OM (%)"  # OM (%), TC (%)
...
output_dir = os.path.join(os.path.dirname(__file__), f"soil_{target_property}_results")
plot_dir = os.path.join(output_dir, f"soil_{target_property}_plots")
excel_output_path = os.path.join(output_dir, f"soil_{target_property}_metrics.xlsx")
...
reference_sheet = "Raw spectral" if "Raw spectral" in sheets_to_run else sheets_to_run[0]
seed = 42
...
print(f"Reference sheet for K-S split: {reference_sheet}")

# Read y once and generate fixed train/test indices from one sheet
...
_, _, train_indices, test_indices = ks_split(X_ref, sample_indices, test_size=0.25)
train_indices = np.asarray(train_indices, dtype=int)
test_indices = np.asarray(test_indices, dtype=int)

print(f"Total samples: {len(y_all)}")
print(f"Train/Test sizes: {len(train_indices)}/{len(test_indices)}")

X_by_sheet = {}
for sheet_name in sheets_to_run:
	df_sheet = pd.read_excel(file_path, sheet_name=sheet_name)
	X_sheet = df_sheet.iloc[:, 1:-2].to_numpy()
	if X_sheet.shape[0] != len(y_all):
		raise ValueError(f"Sample size mismatch in sheet '{sheet_name}': {X_sheet.shape[0]} vs {len(y_all)}")
	X_by_sheet[sheet_name] = X_sheet
...
for sheet_name in sheets_to_run:
	...
	X_sheet = X_by_sheet[sheet_name]
	X_train = X_sheet[train_indices]
	X_test = X_sheet[test_indices]
