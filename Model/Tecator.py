...
selected_sheet = "all"
target_property = "protein"  # moisture, fat, protein
selection_mode = "per_model"
all_preprocessing_sheets = ["Raw", "MSC", "SNV", "SG-2D", "airPLS"]
...
# ==============================================================
# 3. Cross-validated RMSE (with RF grid search)
# ==============================================================
...
def evaluate_cv_rmsecv(model_name, X_tr, y_tr, seed):
    ...
    if model_name == "RF":
        ...
        grid = GridSearchCV(RandomForestRegressor(random_state=seed, n_jobs=-1), param_grid, cv=cv_inner, scoring="neg_mean_squared_error")
        grid.fit(X_tr, y_tr)
        return {
            "RMSECV": np.sqrt(-grid.best_score_),
...
# ==============================================================
# 5. Utilities: safe_name and plotting (4 subplots)
# ==============================================================
...
def plot_true_vs_pred(y_true, results_by_method, preprocessing_name, save_path):
    ...
    methods = ["NIRSpecPFN", "PLSR", "SVR", "RF"]
    ...
    for ax, method in zip(axes, methods):
        ...
        ax.set_title(f"{method} | {preprocessing_name}", fontsize=18)
...
# ==============================================================
# 6. Load sheets and split by C/M (train) vs T (test)
# ==============================================================

dataset_cache = {}
for sheet_name in (all_preprocessing_sheets if selected_sheet == "all" else [selected_sheet]):
...
print('Loaded sheets:', list(dataset_cache.keys()))


# ==============================================================
# 7. Run benchmarking loop and collect metrics
# ==============================================================
...
for sheet_name, (X_train, X_test, y_train, y_test) in dataset_cache.items():
    ...
    for model_name in model_names:
        ...
        all_records.append({
            ...
            "Best_C": cv_info["Best_C"],
            "Best_epsilon": cv_info["Best_epsilon"],
            "Best_max_depth": cv_info.get("Best_max_depth", np.nan),
            "Best_min_samples_leaf": cv_info.get("Best_min_samples_leaf", np.nan),
            "Test_R²": test_info["R2"],
        ...
        print(f"{model_name}: RMSECV={cv_info['RMSECV']:.4f}, R²={test_info['R2']:.4f}, RMSEP={test_info['RMSE']:.4f}, MAE={test_info['MAE']:.4f}, SEP={test_info['SEP']:.4f}, RPD={test_info['RPD']:.4f}")
...
# ==============================================================
# 8. Select best preprocessing & export
# ==============================================================
...
if selection_mode == "per_model":
    ...
    for _, row in selected_for_comparison_df.iterrows():
        print(f"{row['Method']}: preprocessing={row['Preprocessing']}, RMSECV={row['Train_RMSECV']:.4f}, Test_RMSEP={row['Test_RMSEP']:.4f}")
else:
    selected_for_comparison_df = best_global_df.copy()
    print("\n>>> Selection mode: global")
    row = selected_for_comparison_df.iloc[0]
    print(f"Global best: {row['Method']} | {row['Preprocessing']}, RMSECV={row['Train_RMSECV']:.4f}, Test_RMSEP={row['Test_RMSEP']:.4f}")

ks_indices_df = pd.DataFrame({
    "Train_Index": pd.Series(np.arange(len(all_metrics_df))),
    "Test_Index": pd.Series(np.arange(len(all_metrics_df))),
})
